...
void CardRangeUpperRight::DisplayRange(UnitAlly* act_ally, Board* board)
{
}

std::vector<Unit*> CardRangeUpperRight::GetUnitInRange(UnitAlly* act_ally, std::vector<Unit*> all_units)
{
	return std::vector<Unit*>();
}

std::vector<SquarePos> CardRangeUpperRight::GetRangeSquarePos(SquarePos axis_pos)
{
    std::vector<SquarePos> range_square_pos;

    for (int i = 1; i <= range_; ++i) {

        int range_row = axis_pos.row - leave_ - i;  // ã•ûŒü
