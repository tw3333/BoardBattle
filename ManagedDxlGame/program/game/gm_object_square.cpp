...
//CandidateTie
	...
	candidate_tile->mesh_ = dxe::Mesh::CreatePlaneMV({ 79,79,0 });
...
//TargetTile
	...
	target_tile->mesh_ = dxe::Mesh::CreatePlaneMV({ 74, 74,0 });
...
void ObjSquare::Update(float delta_time) {
	...
	//	this->parts_[CandidateTile]->ofs_pos_.y += 1;
	...
	if (this->parts_[CandidateTile]->is_render_) {
		...
		this->parts_[CandidateTile]->ofs_rot_ *= tnl::Quaternion::RotationAxis({ 0, 0, 1 }, tnl::ToRadian(1));
